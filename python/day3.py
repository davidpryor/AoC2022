...
score_dict = {letter: score for score, letter in enumerate(ascii_letters, start=1)}
file_input = '''\
...
def split_rucksacks(rucksacks: str) -> tuple[str, str]:
    size = len(rucksacks) // 2
    return rucksacks[:size], rucksacks[-size:]


def chunk_input(rucksacks: list[str]) -> Generator[None, list[str], None]:
...
if __name__ == '__main__':
    ...
    for line in file_input.splitlines():
        a, b = split_rucksacks(line)
        shared_item = (set(a) & set(b)).pop()
        score += score_dict[shared_item]
    print("task1: ", score)

    score = 0
    for a, b, c in chunk_input(file_input.splitlines()):
        shared_item = (set(a) & set(b) & set(c)).pop()
        score += score_dict[shared_item]
    print("task2: ", score)
